...
class VolcanoBTManager:
    ...
    async def _read_static_attributes(self):
        ...
        try:
            ...
            self.hours_of_operation = await self._read_gatt(UUID_HOURS_OF_OPERATION)
            self.minutes_of_operation = await self._read_gatt(UUID_MINUTES_OF_OPERATION)
            _LOGGER.debug(
                "Static attributes read: BLE Firmware=%s, Serial=%s, Firmware=%s, Auto Shut Off=%s, "
                "Auto Shut Off Setting=%s, LED Brightness=%s, Hours=%s, Minutes=%s",
    ...
    async def _read_gatt(self, uuid):
        ...
        try:
            ...
            return data.decode(errors="ignore") if isinstance(data, bytearray) else str(data)
        except BleakError as e:
            _LOGGER.warning("Error reading GATT characteristic %s: %s", uuid, e)
            return None
